...
function Home(props) {
  ...
  let index = 0;
  let count = 0;
  const min = props.page * 10;
  const max = props.page * 10 + 9;

  while (index < itemList.length) {
    if (tag) {
      console.log(tag in itemList[index]["tag"]);

      if (min <= count && count <= max && tag in itemList[index]["tag"]) {
        showItems.push(itemList[index]);
      }
...
//# sourceMappingURL=index.js.db3144a90c78e5ac32e4.hot-update.js.map